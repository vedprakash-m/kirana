...
└── ⚙️ Settings
    ...
    │   ├── Push Notifications Toggle (Phase 2)
    │   ├── Weekly Digest Toggle
    │   └── Alert Preferences (Phase 2)
    ├── Session Management
    ...
    │   ├── View Audit Logs (90-day history)
    │   ├── Privacy Policy Link
...
### 3.7 Settings: Profile & Account Management
...
│  Danger Zone                                               │
│  ┌────────────────────────────────────────────────────┐   │
│  │ [🗑️ Delete Account]  Permanent action              │   │
│  └────────────────────────────────────────────────────┘   │
│                                                            │
│  [Sign Out]                                                │
└────────────────────────────────────────────────────────────┘
...
│                                                            │
│  Display Name *                                            │
│  [John Doe_________________________________________]       │
│                                                            │
│  Email Address (read-only)                                 │
│  [john.doe@example.com_____________________________]       │
│  💡 Managed by Microsoft Account                           │
│                                                            │
│  Preferences                                               │
│  Timezone                                                  │
│  [America/Los_Angeles ▼____________________________]       │
│                                                            │
│  Currency                                                  │
│  [USD - US Dollar ▼________________________________]       │
│                                                            │
│  [Cancel]                                    [Save]        │
└────────────────────────────────────────────────────────────┘
```

### 3.9 Settings: Active Sessions
...
### 3.10 Settings: Export Data (GDPR Compliance)
...
**Download Flow:**
1. User clicks [Download Export]
2. Loading spinner: "Preparing your data..."
3. Browser download prompt appears with filename: `kirana-export-{date}.json`
4. Success toast: "Data exported successfully"
5. Audit log created: `DATA_EXPORTED` event

### 3.11 Settings: Delete Account (GDPR Right to Deletion)
...
│  • All your items will be removed from shared households   │
│  • Your purchase history will be deleted                   │
│  • You'll be removed from all households                   │
│  • Audit logs will be retained for 90 days (legal req.)    │
│                                                            │
│  Before deleting:                                          │
│  • [Export your data] for your records                     │
│  • Ensure you've removed all payment methods               │
│  • Transfer admin role if you're the only admin            │
│                                                            │
...
**Deletion Flow:**
1. User types "DELETE" (case-sensitive validation)
2. [Delete Account] button becomes enabled
3. Click triggers confirmation dialog: "Are you absolutely sure?"
4. On confirm: Loading screen "Deleting account..."
5. Backend cascade delete:
   - Soft delete items (set deletedAt)
   - Hard delete transactions (privacy)
   - Remove from household members
   - Delete user profile
   - Log `USER_DELETED` event
6. Sign out all sessions
7. Redirect to landing page with message: "Your account has been deleted"

**Component Details:**

**Danger Zone Visual Treatment:**
- Red border, light red background
- Warning icon (⚠️) before action label
- Disabled by default (require confirmation text)
- Hover state shows additional warning
- Accessible: ARIA labels with "danger" role
